...
public class TaschenrechnerOperator {
	...
	/**
	 * Pr�ft die Eingabe auf gleiche Menge der Klammernpaare
	 * @param task
	 * @return
	 */
			...
			if(task.contains("(")) {													
				...
				if(inParentheses.contains("/")||
	 			...
	 			//Wenn ein Operator vorhanden ist 
					 ...
					 task.contains("-")||
					 task.contains("+")){
				//Pr�ft ob Minus ein Vorzeichen ist und alleine steht
				if(task.charAt(0)=='-'){
	...
	/**
	 * Setzen des * vor ge�ffneter Klammer wenn kein anderer Operator vorhanden sein sollte
	 * @param task
	 * @return
	 */
	private String setMultiplyPoint(String task) {
		for(int i= 1;i<task.length();i++) {
			...
			if(openParentheses=='(') {
				...
				/**  
				 * 		@todo - kann man hier ggf. k�rzer abfragen ob "zuPruefen" eine Zahl ist oder nicht?  
				 */
				if(beforePhrase != '+'&&
				   beforePhrase != '-'&&
				   beforePhrase != '*'&&
	...
	/**
	 * Berechnet linkeZahl mit rechteZahl -> gibt summe als String zur�ck
	 * @param task
	 * @param operator
	 * @param indexOperator
	 * @return
	...
	/**
	 * durchsucht task nach Operator und gibt Stelle dessen zur�ck
	 * @param task
	 * @return
	 */
